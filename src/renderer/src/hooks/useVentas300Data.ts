...
export function useVentas300Data() {
  const [propietarios, setPropietarios] = useLocalStorage<Propietario[]>('ventas300_propietarios', mockPropietarios);
  const [propiedades, setPropiedades] = useLocalStorage<Propiedad[]>('ventas300_propiedades', mockPropiedades);
  const [peritos, setPeritos] = useLocalStorage<Perito[]>('ventas300_peritos', mockPeritos);
  const [solicitudes, setSolicitudes] = useLocalStorage<SolicitudVenta[]>('ventas300_solicitudes', mockSolicitudes);
  const [evaluaciones, setEvaluaciones] = useLocalStorage<Evaluacion[]>('ventas300_evaluaciones', mockEvaluaciones);
  const [usuarios] = useLocalStorage<Usuario[]>('ventas300_usuarios', mockUsuarios);
  
  const [usuarioActual, setUsuarioActual] = useLocalStorage<Usuario | null>('ventas300_usuario_actual', usuarios[0]);
  ...
  const crearEvaluacion = (evaluacionData: Omit<Evaluacion, 'id' | 'comisionCalculada' | 'fechaEvaluacion'>) => {
    const perito = peritos.find(p => p.id === evaluacionData.peritoId);
    const comisionCalculada = perito ? (evaluacionData.valorEvaluado * perito.comisionPorcentaje) / 100 : 0;
  ...
  const generateSolicitudNumber = () => {
    ...
    const count = solicitudes.filter(s => s.fechaCreacion.startsWith(`${year}-${month}`)).length + 1;
    return `SOL-${year}-${month}-${String(count).padStart(3, '0')}`;
  };

  // Calcular estadísticas del dashboard
  const getEstadisticasDashboard = (): EstadisticaDashboard => {
    const totalComisiones = evaluaciones.reduce((sum, evaluacion) => sum + evaluacion.comisionCalculada, 0);
    const valorTotalEvaluaciones = evaluaciones.reduce((sum, evaluacion) => sum + evaluacion.valorEvaluado, 0);
